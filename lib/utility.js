...
let scrollLockPrevPos = 0;
export const scrollLock = (enable = true, el, scrollToPrev = false) => {
  if (enable) {
    scrollLockPrevPos = window.pageYOffset;
    el.style.top = `-${scrollLockPrevPos}px`;
    el.classList.add('is-scrollLock');
  } else {
    el.style.removeProperty('top');
    el.classList.remove('is-scrollLock');
    if (scrollToPrev) window.scrollTo(0, scrollLockPrevPos);
  }
};
